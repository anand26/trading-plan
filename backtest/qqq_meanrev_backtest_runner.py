"""
QQQ Mean Reversion (RSI) — Backtest Runner
=============================================
Independent backtest runner for the QQQ RSI(2) Mean Reversion strategy.
Executes LEAN backtests, parses results, stores in SQL.

This is the SWING TRADING companion to the TQQQ Position Trend strategy.
Completely independent runner — does not affect TQQQ or GLD backtesting.

Usage:
    python qqq_meanrev_backtest_runner.py                                      # Single run with defaults
    python qqq_meanrev_backtest_runner.py --start 2010-01-01 --end 2026-02-05  # Custom dates
    python qqq_meanrev_backtest_runner.py --csv qqq_config.csv                 # Batch from CSV
"""
...
@dataclass
class QQQMeanRevConfig:
    """Configuration for a QQQ Mean Reversion backtest run."""
    session_id: str
    start_date: str
    end_date: str
    ...
    # Strategy identifier
    algorithm_name: str = "QQQMeanReversionAlgorithm"
    ...
    # RSI settings
    rsi_period: int = 2               # RSI lookback (2 = Connors' proven value)
    rsi_entry: float = 10.0           # Buy when RSI drops below this
    rsi_exit: float = 70.0            # Sell when RSI rises above this
    
    # Trend filter
    trend_sma: int = 200              # SMA for trend detection
    use_trend_filter: bool = True     # Only trade when QQQ > SMA
    
    # Position sizing
    allocation: float = 0.90          # % of portfolio per trade
...
class QQQMeanRevRunner:
    """
    Runs LEAN backtests for the QQQ Mean Reversion strategy.
    Independent from the TQQQ and GLD strategy runners.
    """
    ...
    def generate_session_id(self, prefix: str = "QQQMREV") -> str:
    ...
    def create_lean_config(self, config: QQQMeanRevConfig) -> Path:
        """Create LEAN config file for QQQ Mean Reversion backtest."""
    ...
    def run_lean_backtest(self, config_path: Path) -> Dict[str, Any]:
        ...
        print(f"[LEAN] Running QQQ Mean Reversion backtest with config: {config_path}")
        
        try:
            env = os.environ.copy()
            
            # Find Python DLL for pythonnet
            python_paths = [
            ...
            # LEAN names the result file after the algorithm, not the session ID
            result_file = self.results_path / config_path.parent.name / "QQQMeanReversionAlgorithm.json"
            log_file = self.results_path / config_path.parent.name / "log.txt"
            ...
            while waited < max_wait_time:
                ...
                if result_file.exists() and result_file.stat().st_size > 1000:
                    # Check the JSON for totalPerformance - this is written LAST by LEAN
                    json_complete = False
                    try:
                        import json as _json
                        with open(result_file, 'r') as rf:
                            result_data = _json.load(rf)
                        ...
                        pass  # JSON still being written
                    
                    if json_complete:
                        print(f"[LEAN] Results fully written - backtest complete")
                        time.sleep(3)  # Extra grace period for file flush
                    ...
                    # Fallback: if process already exited, accept whatever we have
                    if process.poll() is not None:
            ...
            if backtest_completed:
                print("[LEAN] QQQ Mean Reversion backtest completed successfully")
                return {"success": True}
        ...
        try:
            ...
            if exists:
                ...
                cursor.execute("""
                    ...
                    ) VALUES (?, 'BACKTEST', 'QQQ_MEAN_REVERSION', GETUTCDATE(), GETUTCDATE(), 'COMPLETED',
                        ?, ?, ?, ?, ?, ?)
                """, (
            ...
            self.conn.commit()
            print(f"[SQL] Stored QQQ Mean Reversion results: {session_id}")
            return True
        except Exception as e:
            print(f"[SQL] Error: {e}")
            self.conn.rollback()
    ...
    def save_results_to_file(self, parsed_results: Dict, config: QQQMeanRevConfig):
    ...
    def run_backtest(
        ...
        **params
        ...
        """Run a single QQQ Mean Reversion backtest."""
        ...
        print(f"\n{'='*60}")
        print(f"[QQQ MEANREV BACKTEST] Session: {session_id}")
        print(f"[QQQ MEANREV BACKTEST] Period: {start_date} to {end_date}")
        print(f"[QQQ MEANREV BACKTEST] Cash: ${initial_cash:,.2f}")
        if params:
            print(f"[QQQ MEANREV BACKTEST] Params: {params}")
        print(f"{'='*60}\n")
        
        config = QQQMeanRevConfig(
            session_id=session_id,
        ...
        print(f"[COMPLETE] QQQ Mean Reversion backtest done in {total_elapsed:.1f}s | Session: {session_id}")
...
class QQQMeanRevOptimizer:
    """
    CSV-driven batch optimizer for QQQ Mean Reversion parameters.
    ...
    DEFAULTS = {
        'algorithm_name': 'QQQMeanReversionAlgorithm',
        'rsi_period': 2,
        'rsi_entry': 10,
        'rsi_exit': 70,
        'trend_sma': 200,
    ...
    def __init__(self, connection_string: str = None):
        self.runner = QQQMeanRevRunner(connection_string)
    ...
    def load_combinations_from_csv(
        ...
        for idx, row in df.iterrows():
            ...
            params['algorithm_name'] = 'QQQMeanReversionAlgorithm'
            csv_start = row.get('start_date', None)
            csv_end = row.get('end_date', None)
            csv_cash = row.get('initial_cash', None)
            params['start_date'] = start_date or (csv_start if pd.notna(csv_start) else None) or "2000-01-01"
    ...
    def _load_completed_hashes(self):
        """Load hashes of already-completed runs from SQL."""
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE StrategyId = 'QQQ_MEAN_REVERSION'
            """)
    ...
    def run_batch_optimization(
        ...
        results_csv: str = "qqq_meanrev_results.csv",
        source_csv: str = None,
    ):
        """Run all parameter combinations with resume support."""
        self._load_completed_hashes()
        
        run_id = f"QQQ_OPT_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        for i, combo in enumerate(combinations, 1):
            ...
            # Check if already completed (by session_id pattern matching)
            # We use the hash as part of session_id for dedup
            ...
            try:
                result = self.runner.run_backtest(
                    start_date=combo.get('start_date', '2000-01-01'),
...
def main():
    parser = argparse.ArgumentParser(description="QQQ Mean Reversion Backtest Runner")
    ...
    parser.add_argument("--rsi-entry", type=float, default=10)
    parser.add_argument("--rsi-exit", type=float, default=70)
    ...
    parser.add_argument("--results-csv", default="qqq_meanrev_results.csv", help="Output results CSV")
    
    args = parser.parse_args()
    
    if args.csv:
        # Batch mode
        optimizer = QQQMeanRevOptimizer()
        ...
        if result.get("success"):
            stats = result.get("statistics", {})
            print(f"\n{'='*60}")
            print(f"QQQ MEAN REVERSION BACKTEST COMPLETE")
            print(f"{'='*60}")
