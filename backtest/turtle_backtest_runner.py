"""
TQQQ/SQQQ Turtle Trading - Backtest Runner
=============================================
Independent backtest runner for the Turtle Trading strategy.
Executes LEAN backtests, parses results, stores in SQL.

Usage:
    python turtle_backtest_runner.py                           # Single run with defaults
    python turtle_backtest_runner.py --start 2018-06-01 --end 2026-01-14  # Custom dates
    python turtle_backtest_runner.py --csv parameter_combinations_turtle_v1.0.csv  # Batch from CSV

This runner is INDEPENDENT from the pairs strategy runner.
"""
...
class TurtleBacktestConfig:
    """Configuration for a Turtle Trading backtest run."""
    session_id: str
    start_date: str
    end_date: str
    ...
    # Strategy identifier
    algorithm_name: str = "TQQQTurtleAlgorithm"
    ...
    entry_period: int = 20         # Buy on N-day high/low breakout
    exit_period: int = 10          # Exit on M-day low/high
    
    # ATR
    atr_period: int = 20           # ATR lookback
    atr_stop_multiple: float = 2.0  # Stop at 2x ATR from entry
    
    # Risk management
    risk_per_trade: float = 0.005   # 0.5% account risk per unit
    leverage_factor: float = 3.0    # 3x ETF multiplier for vol calc
    
    # Pyramiding
    max_units: int = 3              # Max pyramid units
    pyramid_atr_step: float = 0.5   # Add every 0.5 ATR move in favor
    
    # Position limits
    max_position_pct: float = 0.80  # Max 80% in one direction
    
    # Hold time
    max_hold_days: int = 0          # 0 = no limit
    
    # Trend filter
    use_qqq_filter: bool = False    # QQQ SMA50 filter


class TurtleBacktestRunner:
    """
    Runs LEAN backtests for the Turtle Trading strategy.
    Independent from the pairs strategy runner.
    """
    ...
    def generate_session_id(self, prefix: str = "TURTLE") -> str:
    ...
    def create_lean_config(self, config: TurtleBacktestConfig) -> Path:
        """Create LEAN config file for Turtle Trading backtest."""
        ...
        parameters = {
            ...
            # Donchian channels
            "entry-period": str(config.entry_period),
            "exit-period": str(config.exit_period),
            
            # ATR
            "atr-period": str(config.atr_period),
            "atr-stop-multiple": str(config.atr_stop_multiple),
            
            # Risk
            "risk-per-trade": str(config.risk_per_trade),
            "leverage-factor": str(config.leverage_factor),
            
            # Pyramiding
            "max-units": str(config.max_units),
            "pyramid-atr-step": str(config.pyramid_atr_step),
            ...
            # Trend filter
            "use-qqq-filter": str(config.use_qqq_filter).lower(),
    ...
    def run_lean_backtest(self, config_path: Path) -> Dict[str, Any]:
        ...
        print(f"[LEAN] Running Turtle backtest with config: {config_path}")
        ...
        try:
            ...
            with open(config_path, 'r') as f:
                cfg = json.load(f)
            algo_name = cfg.get('algorithm-type-name', 'TQQQTurtleAlgorithm')
            ...
            if backtest_completed:
                print("[LEAN] Turtle backtest completed successfully")
                return {"success": True}
        ...
        try:
            ...
            if exists:
                ...
                cursor.execute("""
                    ...
                    ) VALUES (?, 'BACKTEST', 'TURTLE_TQQQ', GETUTCDATE(), GETUTCDATE(), 'COMPLETED',
                        ?, ?, ?, ?, ?, ?)
                """, (
            ...
            self.conn.commit()
            print(f"[SQL] Stored Turtle results: {session_id}")
            return True
        except Exception as e:
            print(f"[SQL] Error: {e}")
            self.conn.rollback()
    ...
    def save_results_to_file(self, parsed_results: Dict, config: TurtleBacktestConfig):
    ...
    def run_backtest(
        ...
        **params
        ...
        """Run a single Turtle Trading backtest."""
        ...
        print(f"\n{'='*60}")
        print(f"[TURTLE BACKTEST] Session: {session_id}")
        print(f"[TURTLE BACKTEST] Period: {start_date} to {end_date}")
        print(f"[TURTLE BACKTEST] Cash: ${initial_cash:,.2f}")
        if params:
            print(f"[TURTLE BACKTEST] Params: {params}")
        print(f"{'='*60}\n")
        ...
        print(f"[COMPLETE] Turtle backtest done in {total_elapsed:.1f}s | Session: {session_id}")
...
class TurtleParameterOptimizer:
    """
    CSV-driven batch optimizer for Turtle Trading parameters.
    Resume-capable with hash-based deduplication.
    """
    
    PARAM_COLUMNS = [
        'entry_period', 'exit_period', 'atr_period',
        'risk_per_trade', 'atr_stop_multiple', 'leverage_factor',
        'max_units', 'pyramid_atr_step',
        'max_position_pct', 'max_hold_days',
        'use_qqq_filter',
    ]
    
    DEFAULTS = {
        'algorithm_name': 'TQQQTurtleAlgorithm',
        'entry_period': 20,
        'exit_period': 10,
        'atr_period': 20,
    ...
    def __init__(self, connection_string: str = None):
        self.runner = TurtleBacktestRunner(connection_string)
        ...
        for idx, row in df.iterrows():
            ...
            for col in self.PARAM_COLUMNS:
                if col in row and pd.notna(row[col]):
                    val = row[col]
                    if col == 'use_qqq_filter':
                        val = str(val).lower() in ('true', '1', 'yes')
                    params[col] = val
                elif col in self.DEFAULTS:
                    params[col] = self.DEFAULTS[col]
            
            params['algorithm_name'] = 'TQQQTurtleAlgorithm'
    ...
    def _load_completed_hashes(self) -> set:
        ...
        if results_dir.exists():
            for session_dir in results_dir.iterdir():
                if session_dir.is_dir() and session_dir.name.startswith("TURTLE_"):
    ...
    def run_batch_optimization(
        ...
        results_csv: str = "turtle_optimization_results.csv",
        source_csv: str = None
    ) -> List[Dict[str, Any]]:
...
def main():
    parser = argparse.ArgumentParser(description="Turtle Trading Backtest Runner")
    ...
    parser.add_argument("--pyramid-atr-step", type=float, default=0.5)
    ...
    parser.add_argument("--results-csv", default="turtle_optimization_results.csv", help="Output results CSV")
    
    args = parser.parse_args()
    
    if args.csv:
        # Batch mode
        optimizer = TurtleParameterOptimizer()
        ...
        result = runner.run_backtest(
            ...
            pyramid_atr_step=args.pyramid_atr_step,
            max_hold_days=args.max_hold_days,
            use_qqq_filter=args.use_qqq_filter,
        )
        
        if result.get("success"):
            stats = result.get("statistics", {})
            print(f"\n{'='*60}")
            print(f"TURTLE BACKTEST COMPLETE")
            print(f"{'='*60}")
