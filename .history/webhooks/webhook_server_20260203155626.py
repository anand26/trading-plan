...
class AlpacaExecutor:
    ...
    def buy(self, symbol: str, position_size: float) -> Dict[str, Any]:
        """
        Buy a position using percentage of portfolio.
        
        Args:
            symbol: TQQQ or SQQQ
            position_size: Decimal (0.57 = 57% of portfolio)
        """
        ...
        if qty <= 0:
            raise ValueError(f"Calculated quantity is 0 - equity: {account['equity']}, target: {target_value}")
        
        logger.info(f"Buying {qty} shares of {symbol} @ ~${current_price:.2f} (target ${target_value:.2f})")
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    (EventTime, EventType, Payload, Status, Response, ErrorMessage, SessionType)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    ...
                    json.dumps(response),
                    error,
                    "PAPER" if "paper" in Config.ALPACA_BASE_URL else "LIVE"
                ))
                conn.commit()
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    WHERE SessionType = ? AND EndTime IS NULL
                ...
                if row:
                    ...
                    # Create new session
                    cursor.execute("""
                        INSERT INTO dbo.Sessions (SessionType, StartTime, StrategyId, Notes)
                ...
                cursor.execute("""
                    ...
                    (SessionId, SignalTime, Symbol, SignalType, Price, Quantity, 
                     ConfidenceScore, Notes, IsExecuted)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    session_id,
                    datetime.now(timezone.utc),
