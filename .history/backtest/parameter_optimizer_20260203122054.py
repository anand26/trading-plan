...
class ParameterOptimizer:
    ...
    def load_combinations_from_csv(
        ...
        for idx, row in df.iterrows():
            ...
            # Strategy parameters (v2.1 Hybrid: Mean-Reversion + Volume Confirmation)
            param_columns = [
    ...
    def _get_parameter_defaults(self) -> Dict[str, Any]:
        """Get default parameter values for v2.0 Momentum Strategy."""
        return {
            # ==========================================
            # MOMENTUM v2.0 PARAMETERS (PRIMARY)
            # ==========================================
            'breakout_bars': 3,  # N-bar high/low lookback
            'rsi_period': 14,
            'rsi_momentum_min': 50.0,  # TQQQ: RSI > this
            'rsi_momentum_max': 50.0,  # SQQQ: RSI < this
            'rsi_exit_overbought': 75.0,
            'rsi_exit_oversold': 25.0,
            'position_size': 0.60,  # Single entry 60%
            ...
            # ==========================================
            # LEGACY v1.x PARAMETERS (DEPRECATED)
            # ==========================================
            'rsi_oversold': 30.0,
            'rsi_overbought': 70.0,
        ...
        for i, params in enumerate(combinations):
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    print(f"    [OK] Return: {stats.get('total_return', 0)*100:.2f}%, "
                          f"Sharpe: {stats.get('sharpe_ratio', 0):.2f}, "
                          f"Trades: {stats.get('total_trades', 0)}")
                else:
                    print(f"    [FAIL] Backtest failed: {result.get('error', 'Unknown error')}")
