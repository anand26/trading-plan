...
class BacktestConfig:
    ...
    # ==========================================
    # MOMENTUM STRATEGY v2.0 PARAMETERS
    # ==========================================
    
    # Breakout detection
    breakout_bars: int = 3  # N-bar high/low lookback for breakout
    
    # RSI momentum confirmation (NOT oversold/overbought)
    rsi_period: int = 14
    rsi_momentum_min: float = 50.0  # TQQQ: RSI must be ABOVE this (upward momentum)
    rsi_momentum_max: float = 50.0  # SQQQ: RSI must be BELOW this (downward momentum)
    rsi_exit_overbought: float = 75.0  # Exit when momentum exhausted
    rsi_exit_oversold: float = 25.0  # Exit when momentum exhausted
    
    # Position sizing (single entry, no pyramiding for momentum)
    position_size: float = 0.60  # 60% single position
    
    # Risk management
    stop_loss_pct: float = 0.01  # 1% stop loss
    take_profit_pct: float = 0.015  # 1.5% take profit target
    trailing_stop_pct: float = 0.015  # 1.5% trailing stop
    
    # Volume filter
    require_volume_spike: bool = False  # Optional volume confirmation
    
    # EMA parameters for regime detection
    ema_fast_period: int = 9
    ema_slow_period: int = 21
    
    # Bollinger Bands (used for analysis, not primary signals)
    bb_period: int = 20
    bb_std_dev: float = 2.0
    
    # Legacy parameters (kept for compatibility, but not used in v2.0)
    rsi_oversold: float = 30.0  # DEPRECATED in v2.0
    rsi_overbought: float = 70.0  # DEPRECATED in v2.0
    position_size_level1: float = 0.5  # DEPRECATED in v2.0 (no pyramiding)
    position_size_level2: float = 0.3  # DEPRECATED in v2.0
    position_size_level3: float = 0.2  # DEPRECATED in v2.0
...
class BacktestRunner:
    ...
    def create_lean_config(self, config: BacktestConfig) -> Path:
        ...
        lean_config = {
            ...
            # Parameters passed to algorithm (v2.0 Momentum Strategy)
            "parameters": {
                ...
                # ==========================================
                # Breakout detection
                "breakout-bars": str(config.breakout_bars),
                
                # RSI momentum confirmation
                "rsi-period": str(config.rsi_period),
                "rsi-momentum-min": str(config.rsi_momentum_min),
                "rsi-momentum-max": str(config.rsi_momentum_max),
                "rsi-exit-overbought": str(config.rsi_exit_overbought),
                "rsi-exit-oversold": str(config.rsi_exit_oversold),
                ...
                "trailing-stop-pct": str(config.trailing_stop_pct),
                
                # Volume filter
                "require-volume-spike": str(config.require_volume_spike).lower(),
                ...
                # Legacy parameters (for v1.x compatibility)
