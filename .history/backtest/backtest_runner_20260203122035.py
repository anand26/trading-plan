...
class BacktestConfig:
    ...
    # ==========================================
    # HYBRID STRATEGY v2.1 PARAMETERS
    # Mean-Reversion Core + Volume Confirmation + Wider Stops
    # ==========================================
    
    # RSI Mean-Reversion Parameters (PRIMARY)
    rsi_period: int = 14
    rsi_oversold: float = 28.0  # Entry threshold (best from v1.7: RSI 28)
    rsi_overbought: float = 72.0  # Exit threshold
    rsi_extreme_oversold: float = 20.0  # Pyramid level 3 entry
    rsi_extreme_overbought: float = 80.0  # Strong exit signal
    
    # Bollinger Bands Parameters
    bb_period: int = 20
    bb_std_dev: float = 2.0
    
    # Position Sizing (Pyramiding - mean reversion style)
    position_size_level1: float = 0.50  # 50% initial
    position_size_level2: float = 0.30  # 30% add
    position_size_level3: float = 0.20  # 20% final
    pyramid_level2_profit: float = 0.003  # 0.3% profit to add
    
    # Risk Management - WIDER STOPS FOR OPTION 2
    stop_loss_pct: float = 0.025  # 2.5% stop (wider than v1.x 2%)
    take_profit_pct: float = 0.04  # 4% take profit target
    trailing_stop_pct: float = 0.0  # Disabled by default
    
    # OPTION 2 HYBRID: Volume Confirmation
    require_volume_spike: bool = True  # Require volume spike for entry
    volume_spike_mult: float = 1.5  # 1.5x average volume
...
class BacktestRunner:
    ...
    def create_lean_config(self, config: BacktestConfig) -> Path:
        ...
        lean_config = {
            ...
            # Parameters passed to algorithm (v2.1 Hybrid Strategy)
            "parameters": {
                ...
                # ==========================================
                # HYBRID v2.1 PARAMETERS (Mean-Reversion + Volume)
                # ==========================================
                # RSI Mean-Reversion (PRIMARY)
                "rsi-period": str(config.rsi_period),
                "rsi-oversold": str(config.rsi_oversold),
                "rsi-overbought": str(config.rsi_overbought),
                "rsi-extreme-oversold": str(config.rsi_extreme_oversold),
                "rsi-extreme-overbought": str(config.rsi_extreme_overbought),
                ...
                "pyramid-level2-profit": str(config.pyramid_level2_profit),
                ...
                # OPTION 2: Volume spike confirmation
                "require-volume-spike": str(config.require_volume_spike).lower(),
                "volume-spike-mult": str(config.volume_spike_mult),
