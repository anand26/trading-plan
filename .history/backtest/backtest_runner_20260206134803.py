...
class BacktestConfig:
    ...
    # Z-score lookback (number of consolidated bars)
    ...
    min_bars_between: int = 6    # Min bars between trades
    bar_period_minutes: int = 5  # Consolidator bar period (5/15/30)
    
    # ==========================================
    # TREND FILTER (QQQ 50/200-day SMA)
    # ==========================================
    trend_filter_enabled: str = "true"    # Enable QQQ trend filter
    sma_fast_period: int = 50             # Fast SMA period
    sma_slow_period: int = 200            # Slow SMA period
    
    # ==========================================
    # DRAWDOWN CIRCUIT BREAKER
    # ==========================================
    daily_loss_limit_pct: float = 0.02    # Halt trading if daily loss > 2%
    max_drawdown_pct: float = 0.25        # Reduce size if DD > 25%
    drawdown_position_scale: float = 0.50 # Scale factor when in drawdown
    
    # ==========================================
    # VOLATILITY SCALING (optional)
    # ==========================================
    enable_vol_scaling: str = "false"     # Disabled by default
    vol_scale_high_thresh: float = 2.5    # QQQ ATR% > 2.5% = high vol
    vol_scale_med_thresh: float = 1.5     # QQQ ATR% > 1.5% = medium vol
    vol_scale_high_factor: float = 0.44
    vol_scale_med_factor: float = 0.70
    
    # ==========================================
    # PROFIT TARGET & TRAILING STOP
    # ==========================================
    take_profit_pct: float = 0.0          # 0 = disabled
    trailing_activation_pct: float = 0.01 # 1% gain activates trailing
    trailing_stop_pct: float = 0.0        # 0 = disabled
    
    # ==========================================
    # WARMUP
    # ==========================================
    warmup_days: int = 210                # 200+ days for SMA200 warmup
    ...
    def create_lean_config(self, config: BacktestConfig) -> Path:
        ...
        if algo_name == "TQQQSQQQPairsAlgorithm":
            ...
            parameters = {
                ...
                "bar-period-minutes": str(config.bar_period_minutes),
                
                # Trend filter
                "trend-filter-enabled": str(config.trend_filter_enabled),
                "sma-fast-period": str(config.sma_fast_period),
                "sma-slow-period": str(config.sma_slow_period),
                
                # Drawdown circuit breaker
                "daily-loss-limit-pct": str(config.daily_loss_limit_pct),
                "max-drawdown-pct": str(config.max_drawdown_pct),
                "drawdown-position-scale": str(config.drawdown_position_scale),
                
                # Volatility scaling
                "enable-vol-scaling": str(config.enable_vol_scaling),
                "vol-scale-high-thresh": str(config.vol_scale_high_thresh),
                "vol-scale-med-thresh": str(config.vol_scale_med_thresh),
                "vol-scale-high-factor": str(config.vol_scale_high_factor),
                "vol-scale-med-factor": str(config.vol_scale_med_factor),
                
                # Profit target & trailing stop
                "take-profit-pct": str(config.take_profit_pct),
                "trailing-activation-pct": str(config.trailing_activation_pct),
                "trailing-stop-pct": str(config.trailing_stop_pct),
                
                # Warmup
                "warmup-days": str(config.warmup_days),
