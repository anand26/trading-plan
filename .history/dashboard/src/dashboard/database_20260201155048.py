...
def get_optimization_results(
    ...
    sql = f"""
        SELECT TOP {limit}
            ...
            TRY_CAST(JSON_VALUE(b.ParametersJson, '$.rsi_period') AS INT) as rsi_period,
            ...
            TRY_CAST(JSON_VALUE(b.ParametersJson, '$.bb_period') AS INT) as bb_period,
...
def get_optimization_summary(run_id: Optional[str] = None) -> dict:
    ...
    sql = f"""
        SELECT 
            ...
            COUNT(CASE WHEN b.SharpeRatio > 1 THEN 1 END) as good_sharpe_runs
        FROM BacktestOutcomes b
        {run_filter}
    """
