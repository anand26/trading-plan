...
from dashboard.database import get_trades, get_trade_summary
from dashboard.components.charts import create_trade_scatter, create_hourly_performance_chart
...
def render_trades():
    ...
    if side_filter != "All":
        trades_df = trades_df[trades_df["Direction"] == side_filter]
    ...
    with col2:
        ...
        hourly_wr = trades_df.groupby("Hour").apply(
            lambda x: (x["NetPnL"] > 0).sum() / len(x) * 100
        ).reset_index()
    ...
    with col1:
        ...
        trades_df["Duration_Min"] = trades_df["DurationMinutes"]
    ...
    with col2:
        ...
        fig = px.scatter(
            ...
            y="NetPnL",
            color="Symbol",
            hover_data=["Direction", "EntryTime"],
            opacity=0.6
        )
    ...
    trades_df["IsWin"] = trades_df["NetPnL"] > 0
    ...
    display_df["PnLPercent"] = display_df["PnLPercent"].apply(lambda x: f"{x:.2f}%" if x is not None else "N/A")
