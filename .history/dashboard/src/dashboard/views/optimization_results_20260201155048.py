...
def render_run_optimization_tab():
    ...
    with col1:
        can_run = csv_path is not None and end_date > start_date and not st.session_state.opt_running
        
        if st.button(
            "▶️ Start Optimization" if not st.session_state.opt_running else "⏳ Running...",
            type="primary",
    ...
    with col2:
        if st.session_state.opt_running:
            st.info("🔄 Optimization in progress... Check output below.")
        elif st.session_state.opt_progress > 0:
            st.success(f"✅ Completed {st.session_state.opt_progress} backtests")
    ...
    # Progress and output
    if st.session_state.opt_running or st.session_state.opt_output:
        st.markdown("### 📋 Output Log")
        
        output_container = st.container()
        with output_container:
            # Show output as code block
            output_text = "\n".join(st.session_state.opt_output[-50:])  # Last 50 lines
            st.code(output_text if output_text else "Waiting for output...", language="text")
...
def run_optimization_process(csv_path: str, start_date: str, end_date: str, resume: bool = True):
    """Run the optimization process and update session state."""
    import subprocess
    import sys
    ...
    try:
        # Run as subprocess with output capture
        process = subprocess.Popen(
            cmd,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            text=True,
            cwd=str(base_path / "backtest"),
            bufsize=1
        )
        ...
        for line in iter(process.stdout.readline, ''):
            ...
            if line:
                ...
                st.session_state.opt_output = output_lines[-100:]  # Keep last 100 lines
                
                # Parse progress from output
                if "Testing:" in line or "Skipping" in line:
                    # Try to extract progress
                    if "/" in line:
                        try:
                            parts = line.split("]")[0].split("[")[1]
                            current, total = parts.split("/")
                            st.session_state.opt_progress = int(current)
                            st.session_state.opt_total = int(total)
                        except:
                            pass
        ...
        if process.returncode == 0:
            st.session_state.opt_output.append("✅ Optimization completed successfully!")
        else:
            st.session_state.opt_output.append(f"❌ Optimization failed with code {process.returncode}")
            
    except Exception as e:
        st.session_state.opt_running = False
        st.session_state.opt_output.append(f"❌ Error: {str(e)}")
