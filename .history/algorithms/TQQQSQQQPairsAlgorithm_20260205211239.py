...
class TQQQSQQQPairsAlgorithm(QCAlgorithm):
    ...
    def EnterPosition(self, symbol, symbol_name: str, zscore: float) -> None:
        ...
        # Use SetHoldings with liquidateExistingHoldings=True and immediate=True to avoid MarketOnOpen issues
        self.SetHoldings(symbol, self.position_size, liquidateExistingHoldings=True, tag=f"{symbol_name} Entry: Z={zscore:.2f}")  # type: ignore
