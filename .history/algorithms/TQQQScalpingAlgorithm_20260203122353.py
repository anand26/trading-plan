...
class TQQQScalpingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):  # type: ignore
        ...
        self.stop_loss_pct = float(self.GetParameter("stop-loss-pct") or self.GetParameter("stop_loss_pct") or 0.025)  # type: ignore  # 2.5% default (wider for hybrid)
        self.take_profit_pct = float(self.GetParameter("take-profit-pct") or self.GetParameter("take_profit_pct") or 0.04)  # type: ignore  # 4% take profit target
        ...
        # Volume Filter - OPTION 2 HYBRID: Require volume spike for entry confirmation
        self.require_volume_spike = str(self.GetParameter("require-volume-spike") or self.GetParameter("require_volume_spike") or "true").lower() == "true"  # type: ignore
        ...
        self.Log(f"[PARAMS] Volume Spike Required: {self.require_volume_spike}, Multiplier: {self.volume_spike_multiplier}")  # type: ignore
        self.Log(f"[PARAMS] BB Std Dev: {self.bb_std_dev}, Stop Loss: {self.stop_loss_pct}, Take Profit: {self.take_profit_pct}")  # type: ignore
        self.Log(f"[PARAMS] RSI Period: {self.rsi_period}, BB Period: {self.bb_period}")  # type: ignore
        self.Log(f"[PARAMS] EMA Fast: {self.ema_fast_period}, EMA Slow: {self.ema_slow_period}")  # type: ignore
        self.Log(f"[PARAMS] Position Sizes: L1={self.position_size_level1}, L2={self.position_size_level2}, L3={self.position_size_level3}")  # type: ignore
    ...
    def ProcessTQQQSignals(self, bar: TradeBar) -> None:
        ...
        if current_qty > 0:
            ...
            # 1. Direct percentage target (configurable take_profit_pct)
            # 2. RSI reversal (crosses above 50 or reaches overbought)
            # 3. Price above VWAP with profit
            take_profit = False
            exit_reason = ""
            
            # Check percentage-based take profit first
            if profit_pct >= self.take_profit_pct:
                take_profit = True
                exit_reason = f"Take profit target hit ({profit_pct:.2%} >= {self.take_profit_pct:.2%})"
            elif rsi > 70:
                take_profit = True
        ...
        if current_qty == 0:
            ...
            # - OPTION 2 HYBRID: Volume spike confirmation (configurable)
            volume_confirmed = volume_spike or not self.require_volume_spike
            
            if rsi < self.rsi_oversold and price <= bb_middle * 1.02 and volume_confirmed:
                target_pct = self.position_size_level1
                self.SetHoldings(symbol, target_pct, tag=f"TQQQ Entry L1: RSI={rsi:.1f}, Vol={'Y' if volume_spike else 'N'}")
                ...
                self.LogTrade("TQQQ", "ENTRY_L1", price, new_qty, 0, 
                             f"RSI={rsi:.1f}, BB_Middle={bb_middle:.2f}, VolSpike={volume_spike}")
                ...
                if self.webhook_simulator:
                    self.webhook_simulator.trigger_signal_webhook(
                        ...
                        trigger_conditions={
                            ...
                            "volume_required": self.require_volume_spike,
                            "quantity": new_qty,
    ...
    def ProcessSQQQSignals(self, bar: TradeBar) -> None:
        ...
        if current_qty == 0:
            ...
            if rsi < self.rsi_oversold and price <= bb_lower * 1.01 and volume_confirmed:
                target_pct = self.position_size_level1
                self.SetHoldings(symbol, target_pct, tag=f"SQQQ Entry L1: RSI={rsi:.1f}, Vol={'Y' if volume_spike else 'N'}")
                ...
                self.LogTrade("SQQQ", "ENTRY_L1", price, new_qty, 0,
                             f"RSI={rsi:.1f}, BB_Lower={bb_lower:.2f}, VolSpike={volume_spike}")
                self.daily_trades_count += 1
