...
Version: 4.0.0 (Structural Improvements)
"""
...
class TQQQSQQQPairsAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):  # type: ignore
        ...
        # Z-score lookback period (number of consolidated bars)
        ...
        self.entry_zscore_threshold = float(self.GetParameter("entry-zscore") or self.GetParameter("entry_zscore") or 2.0)  # type: ignore
        ...
        self.min_bars_between_trades = int(float(self.GetParameter("min-bars-between") or self.GetParameter("min_bars_between") or 6))  # type: ignore
        
        # Configurable bar period (minutes) for TQQQ/SQQQ consolidation
        self.bar_period_minutes = int(float(self.GetParameter("bar-period-minutes") or self.GetParameter("bar_period_minutes") or 5))  # type: ignore
        
        # =====================================================
        # PROFIT TARGET & TRAILING STOP (let winners run)
        # =====================================================
        # Take profit: exit at this % gain (0 = disabled, use Z-score exit only)
        self.take_profit_pct = float(self.GetParameter("take-profit-pct") or self.GetParameter("take_profit_pct") or 0.0)  # type: ignore
        
        # Trailing stop: activated after position gains trailing_activation_pct,
        # then trails at trailing_stop_pct below the high-water mark
        self.trailing_activation_pct = float(self.GetParameter("trailing-activation-pct") or self.GetParameter("trailing_activation_pct") or 0.01)  # type: ignore  # 1% gain activates trailing
        self.trailing_stop_pct = float(self.GetParameter("trailing-stop-pct") or self.GetParameter("trailing_stop_pct") or 0.0)  # type: ignore  # 0 = disabled
        ...
        self.vol_scale_high_thresh = float(self.GetParameter("vol-scale-high-thresh") or self.GetParameter("vol_scale_high_thresh") or 2.5)  # type: ignore  # QQQ ATR% > 2.5% = high vol
        self.vol_scale_med_thresh = float(self.GetParameter("vol-scale-med-thresh") or self.GetParameter("vol_scale_med_thresh") or 1.5)  # type: ignore  # QQQ ATR% > 1.5% = medium vol
        ...
        # Log parameters
        self.Log(f"[PARAMS] Bar Period: {self.bar_period_minutes}min, Z-score Lookback: {self.zscore_lookback} bars")  # type: ignore
        self.Log(f"[PARAMS] Entry Z-score: ±{self.entry_zscore_threshold}, Exit Z-score: ±{self.exit_zscore}")  # type: ignore
        self.Log(f"[PARAMS] Stop Z-score: ±{self.stop_zscore}, Stop Loss: {self.stop_loss_pct:.1%}")  # type: ignore
        self.Log(f"[PARAMS] Position Size: {self.position_size:.0%}, Min Bars Between: {self.min_bars_between_trades}")  # type: ignore
        ...
        self.Log(f"[PARAMS] Vol Scaling: {'ON' if self.enable_vol_scaling else 'OFF'} (thresholds: {self.vol_scale_med_thresh}/{self.vol_scale_high_thresh}%)")  # type: ignore
        self.Log(f"[PARAMS] Take Profit: {self.take_profit_pct:.1%}, Trailing Stop: {self.trailing_stop_pct:.1%} (activate at {self.trailing_activation_pct:.1%})")  # type: ignore
        ...
        # =====================================================
        # CONSOLIDATORS (configurable bar period for TQQQ/SQQQ)
        # =====================================================
        self.bar_period = timedelta(minutes=self.bar_period_minutes)
        ...
        self.entry_zscore_value: float = 0.0  # Z-score at entry (renamed to avoid collision)
        self.bars_since_trade: int = 0
        self.high_water_price: float = 0.0    # Highest price since entry (for trailing stop)
        self.trailing_stop_active: bool = False  # Whether trailing stop has been activated
        ...
        if HAS_SQL_CONNECTOR and self.sql_enabled:
            try:
                ...
                params_snapshot = {
                    ...
                    "entry_zscore": self.entry_zscore_threshold,
    ...
    def EnterPosition(self, symbol, symbol_name: str, zscore: float) -> None:
        ...
        self.entry_zscore_value = zscore
        self.bars_since_trade = 0
        self.daily_trades_count += 1
        self.high_water_price = current_price  # Initialize trailing stop tracking
    ...
    def ProcessExitSignals(self, zscore: float) -> None:
        """Check for exit signals with trailing stop and profit target support."""
        ...
        # Exit 1: Percentage stop loss (hard stop — check first)
        ...
        # Exit 3: Take profit target (if configured)
        elif self.take_profit_pct > 0 and pnl_pct >= self.take_profit_pct:
            exit_reason = f"Take profit: {pnl_pct:.2%}"
        
        # Exit 4: Trailing stop (if configured and activated)
        elif self.trailing_stop_pct > 0:
            # Activate trailing stop once we've gained enough
            if not self.trailing_stop_active and pnl_pct >= self.trailing_activation_pct:
                self.trailing_stop_active = True
                self.Log(f"[TRAIL] Activated for {symbol_name} at {pnl_pct:.2%} gain, HWM=${self.high_water_price:.2f}")
            
            # Check if price dropped below trailing threshold from high-water
            if self.trailing_stop_active and self.high_water_price > 0:
                drop_from_high = (self.high_water_price - current_price) / self.high_water_price
                if drop_from_high >= self.trailing_stop_pct:
                    exit_reason = f"Trailing stop: {drop_from_high:.2%} from HWM ${self.high_water_price:.2f}"
        
        # Exit 5: Z-score normalized (mean reversion complete)
        # Only if trailing stop is NOT active (let winners run when trailing)
        if exit_reason is None and not self.trailing_stop_active:
    ...
    def ExitPosition(self, symbol, symbol_name: str, zscore: float, pnl_pct: float, reason: str) -> None:
        ...
        if self.entry_time:
            ...
            trade_record = TradeRecord(
                ...
                pnl=pnl,
                pnl_pct=pnl_pct,
                entry_zscore=self.entry_zscore_value,
                exit_zscore=zscore,
        ...
        if self.db and self.entry_time:
            self.db.log_trade(
                ...
                entry_time=self.entry_time,
                exit_time=self.Time,
                entry_reason=f"Z-score entry: {self.entry_zscore_value:.2f}",
                exit_reason=reason
            )
